...
rdm r; // for genrating random numbers　乱数生成用
...
//これは配列による初期化の例です。MTRand(seed) を任意の 32 ビット整数と共にシードとして使用して、より簡単な初期化を行うことができます。
...
//マップが受信されるまで待ちます。マップが受信されると、mapData.header.seq は < 1 にはなりません
...
//マーカーの動作を設定します。 オプションは ADD、DELETE、および ROS Indigo の新機能: 3 (DELETEALL)
...
//init_map_x=50; //mazemap専用
//init_map_y=50; //mazemap専用
//Xstartx=0; //mazemap専用
//Xstarty=0; //mazemap専用

//---------------------rvizのpublish point設定 end----------------------------
