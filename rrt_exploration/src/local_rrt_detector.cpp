...
// generate the same numbers as in the original C test program
  ...
  ros::param::param<float>(ns+"/eta", eta, 0.5); //locat tree成長率
...
int i=0;
float xr,yr,n,local_init_map,LTparam,LTstartx,LTstarty;
local_init_map=5;//LT成長範囲のパラメータ
LTparam=100;//LT成長方向指定確率
...
n=drand()*100;
if(n<LTparam){
LTstartx=V[0][0]; LTstarty=V[0][1];
xr=(drand()*local_init_map)-(local_init_map*0.5)+LTstartx;
yr=(drand()*local_init_map)-(local_init_map*0.5)+LTstarty;
}else{
	//------------
